...
const InteractiveTerminal = ({ commands, cwd, onComplete }: { commands: string[], cwd: string, onComplete: () => void }) => {
    ...
    const runSequence = async () => {
        ...
        for (const cmd of commands) {
            ...
            // BACKEND: Execute
            try {
                const res = await api.executeGit(cmd, cwd);
                const outputLines = res.output.split('\n');
                setLines(prev => [...prev, ...outputLines]);
                
                if (!res.success) {
                    setLines(prev => [...prev, `Error: Command failed with status ${res.output}`]);
                    setIsRunning(false);
                    return; // Stop on error
                }
            } catch (e) {
                setLines(prev => [...prev, `System Error: ${e}`]);
                setIsRunning(false);
                return;
            }
        }
    ...
    return (
        <div className="bg-[#0F172A] rounded-xl border border-slate-700 overflow-hidden shadow-2xl my-4 flex flex-col h-[300px]">
            ...
            <div className="flex items-center justify-between px-4 py-2 bg-slate-800 border-b border-slate-700 shrink-0">
                ...
                <div className="text-[10px] text-slate-400 font-mono">jovyan@workspace:{cwd}</div>
            </div>
